...
class ConeMapping():
    def __init__(self, state):
        rospy.Subscriber("/cluster_filter", PoseArray,
                         callback=self.ClusterCallback)
        self.marker_pub = rospy.Publisher(
            "/marker/obejct", MarkerArray, queue_size=1)
    ...
    def ClusterCallback(self, msg):
        ...
        for i in msg.poses:
            ...
            map_x = self.state.x + \
                (i.position.x + 0.5) * m.cos(self.state.yaw) - \
                i.position.y * m.sin(self.state.yaw)
            map_y = self.state.y + \
                (i.position.x + 0.5) * m.sin(self.state.yaw) + \
                i.position.y * m.cos(self.state.yaw)
            ...
            if n >= self.count:
                ...
                if len(self.list3) == 0:
                    ...
                    if min(dis_arr) <= self.r:
                        self.list3[min_idx][0] = (
                            self.list3[min_idx][0] + i[0]) / 2
                        self.list3[min_idx][1] = (
                            self.list3[min_idx][1] + i[1]) / 2
    ...
    def MarkObject(self):
        ...
        for i in range(len(self.list4)):
            ...
            marker.pose.position = Point(
                self.list4[i][0], self.list4[i][1], 0.)
