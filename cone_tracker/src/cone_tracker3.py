        ...
        if self.loop_closure is True:
                ...
                if p_curve:
                    ...
                    if p_distance:
                        ...
                        if self.mapper.map.map[i][j].count <= 5:
                            grid_cost = self._calculateGridCost(
                                self.node.data, point) * self.g_gain
            ...
            if best_point is None or min_cost == float("inf"):
                ...
                if self.node.parent is not None:
                    ...
                    theta = abs(np.dot(car_vec, point_vec) /
                                (np.hypot(point_vec[0], point_vec[1])))
            ...
            new_node = Node(data=best_point, idx=(
                self.node.idx + 1), parent=self.node)
