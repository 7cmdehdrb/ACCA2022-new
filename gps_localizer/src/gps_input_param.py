...
class get_params(object):
    def __init__(self):
        ...
        self.Odom_sub = rospy.Subscriber("/odometry/global", Odometry, callback=self.OdomCallback)
    ...
    def handleData(self):
        ...
        map_y = []
        for i in range(5):
            utm_x.append(self.UTM_x)
            utm_y.append(self.UTM_y)
            map_x.append(self.odom_x)
            map_y.append(self.odom_y)
            print("wait...")
            sleep(1)
        self._utm_x = np.mean(utm_x)
        self._utm_y = np.mean(utm_y)
        self._map_x = np.mean(map_x)
        self._map_y = np.mean(map_y)

        return float(self._utm_x), float(self._utm_y), float(self._map_x), float(self._map_y)
