...
try:
    ...
    rospy.logfatal(ex)
    rospy.logfatal("Import Error : Obstacle_final - erp42_control")
...
class obstacle(object):
    def __init__(self, state):
        ...
        xs = [60, 70]
        ys = [0, -30]
        left_cx, left_cy, left_cyaw, _, _ = calc_spline_course(
            xs[:], ys[:], ds=0.1)
        ...
        self.obstacle_sub = rospy.Subscriber(
            "/adaptive_clustering/poses", PoseArray, callback=self.ObstacleCallback)
        ...
        self.obs_pub_obs = rospy.Publisher(
            "obstacle_position", MarkerArray, queue_size=10)
        self.obs_pub_way = rospy.Publisher(
            "waypoint_position", MarkerArray, queue_size=10)
    ...
    def GetDistance(self, point1, point2):
        distance = m.sqrt((point1[0] - point2[0]) **
    ...
    def DetectObstacle(self):
        ...
        for i in self.ObsMsg.poses:
            ...
            map_x = self.state.x + velodyne_x * \
                m.cos(self.state.yaw) - velodtne_y * m.sin(self.state.yaw)
            map_y = self.state.y + velodyne_x * \
                m.sin(self.state.yaw) + velodtne_y * m.cos(self.state.yaw)
            ...
            if angle <= self.detect_obs_angle and dis_path <= self.detect_obs_range:
                ...
                if len(self.obs_mapping) != 0:
                    ...
                    if prox_dis <= self.prox_dis:
                        map_x, map_y, new_count = (map_x + self.obs_mapping[prox_idx][0]) / 2, (
                            map_y + self.obs_mapping[prox_idx][1]) / 2,  self.obs_mapping[prox_idx][2] + 1
    ...
    def CreateWaypoint(self):
        ...
        for obs in self.obstacle:
            ...
            a, b = obs[0], obs[1]
            target_idx = self.calc_target_index(
                self.PathMsg.cx, self.PathMsg.cy, [a, b])
            p = -1. / self.PathMsg.cyaw[target_idx]
            c = b - p * a

            t1 = (2*a + 2*p*b - 2*p*c + m.sqrt((-2*a - 2*p*b + 2*p*c)**2 - 4 *
                  (1+p**2) * (a**2 + b**2 + c**2 - 2*b*c - self.r**2)))/(2 * (1 + p**2))
            t2 = (2*a + 2*p*b - 2*p*c - m.sqrt((-2*a - 2*p*b + 2*p*c)**2 - 4 *
                  (1+p**2) * (a**2 + b**2 + c**2 - 2*b*c - self.r**2)))/(2 * (1 + p**2))

            waypoint1 = [t1, p * t1 + c]
            waypoint2 = [t2, p * t2 + c]

            dis_obs_left = self.GetDistance(
                [a, b], [self.left[target_idx][0], self.left[target_idx][1]])
            dis_left_path = self.GetDistance([self.left[target_idx][0], self.left[target_idx][1]], [
                                             self.path[target_idx][0], self.path[target_idx][1]])

            dis_way1_left = self.GetDistance([waypoint1[0], waypoint1[1]], [
                                             self.left[target_idx][0], self.left[target_idx][1]])
            # dis_way1_right = self.GetDistance([waypoint1[0], waypoint1[1]], [self.right[target_idx][0], self.right[target_idx][1]])
            dis_way2_left = self.GetDistance([waypoint2[0], waypoint2[1]], [
                                             self.left[target_idx][0], self.left[target_idx][1]])
    ...
    def CreatPath(self):
        ...
        state_tar_idx = self.calc_target_index(self.PathMsg.cx, self.PathMsg.cy, [
                                               self.state.x, self.state.y])

        if len(self.waypoint_arr) != 0:
            max_tar_idx = -1
            self.waypoint_arr.sort(key=lambda x: self.calc_target_index(
                self.PathMsg.cx, self.PathMsg.cy, x))

            if len(self.waypoint_arr) >= 2:

                min_tar_idx = self.calc_target_index(
                    self.PathMsg.cx, self.PathMsg.cy, self.waypoint_arr[0])
                max_tar_idx = self.calc_target_index(
                    self.PathMsg.cx, self.PathMsg.cy, self.waypoint_arr[-1])
                ...
                if max_tar_idx + 40 >= state_tar_idx:
                    self.cx, self.cy, self.cyaw, _, _ = calc_spline_course(
                        xs[:], ys[:], ds=0.1)
                else:
                ...
                max_tar_idx = self.calc_target_index(
                    self.PathMsg.cx, self.PathMsg.cy, self.waypoint_arr[0])
                xs = [self.path[max_tar_idx - 40][0], self.waypoint_arr[0]
                      [0], self.path[max_tar_idx + 40][0]]
                ys = [self.path[max_tar_idx - 40][1], self.waypoint_arr[0]
                      [1], self.path[max_tar_idx + 40][1]]
    ...
    def publishPoint(self, position, color=ColorRGBA, scale=Vector3):
        ...
        for i in range(len(position)):
            ...
            marker.pose.position = Point(
                position[i][0], position[i][1], 0.)  # cone.point
    ...
    def main(self):
        ...
        if len(self.obstacle) != 0:
            ...
            self.publishPoint(self.obstacle, ColorRGBA(
                1., 0., 0., 1.), Vector3(0.5, 0.5, 0.5))
            self.publishWaypoint(self.waypoint_arr, ColorRGBA(
                1., 1., 0., 1.), Vector3(0.2, 0.2, 0.2))
