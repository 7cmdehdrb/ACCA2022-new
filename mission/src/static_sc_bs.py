...
class Obstacle(object):
    ...
    def __init__(self, state):
        ...
        path_data = pd.read_csv("/home/acca/catkin_ws/src/ACCA2022-new/mission/data/sc/bs/static_path.csv")
        center_data = pd.read_csv("/home/acca/catkin_ws/src/ACCA2022-new/mission/data/sc/bs/static_center.csv")
        left_data = pd.read_csv("/home/acca/catkin_ws/src/ACCA2022-new/mission/data/sc/bs/static_left.csv")
    ...
    def ObstacleCallback(self, msg):
        ...
        self.list1 = []
        for i in msg.poses:
            if m.sqrt((i.position.x) **2 + (i.position.y) ** 2 ) < 25. and (1.57 >= m.atan2(i.position.y, i.position.x)) and (m.atan2(i.position.y, i.position.x) > -1.57):
    ...
    def Mapping(self):
        ...
        for i in self.list3:
            ...
            if i[2] >= self.threshold and self.theta > theta and dis <= 30:
    ...
    def CreatPath(self):
        ...
        if len(self.waypoint_arr) != 0:
            ...
            if len(self.waypoint_arr) >= 2:
                ...
                xs = [self.state.x, self.waypoint_arr[0][0], self.path[max_tar_idx + 40][0]]
                ys = [self.state.y, self.waypoint_arr[0][1], self.path[max_tar_idx + 40][1]]
    ...
    def main(self):
        ...
        if len(self.list4) != 0:
            self.obs_state = True
            desire_speed = 5.
            self.msg.Speed = self.pid.PIDControl(desire_speed)
        else:
            self.obs_state = False
            desire_speed = 5.
            self.msg.Speed = self.pid.PIDControl(desire_speed)
