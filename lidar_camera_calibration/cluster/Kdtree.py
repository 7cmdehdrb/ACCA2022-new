...
class KdTree_class:
    ...
    def insert_points(self, point_array, display_output=False):
        """
        :param pcd_dataframe: dataframe containing (x,y,z) coordinates data
        :return:
        """
        for i in range(len(point_array)):
            x,y,z,point_id = point_array[i][0],point_array[i][1],point_array[i][2],i
            point = (point_array[i][0],point_array[i][1],point_array[i][2])
            level = 2
    ...
    def __dict_key(number):
        ...
        try:
            ...
            raise Exception("Incorrect Level({number}) Assignment.")
