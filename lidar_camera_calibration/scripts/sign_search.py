...
class SignSearch(object):
        ...
        if len(temp_size) > 0:
            ...
            for i in range(len(temp_size)):
                if temp_size[i] >= near_sign - 10:  # number is not perfect
                    boxclass.append(int(temp_class[i]))
    ...
    def choosenum(self, num):
        num_str = str(num)
        if num_str in self.stop_0:
            self.sign[-1] = 0
        elif num_str in self.left_1:
            self.sign[-1] = 1
        elif num_str in self.straight_2:
            self.sign[-1] = 2
        else:
            self.sign[-1] = 3
