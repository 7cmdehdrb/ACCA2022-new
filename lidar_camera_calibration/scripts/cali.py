...
def project_point_cloud(velodyne, img_msg, box_data, image_pub, points_pub):
    ...
    points3D = np.delete(points3D,[3],1)

    # for i in range(len(points_inbox)):
    #     box_3D.append(points3D[points_inbox[i]])
    ...
    for i in range(len(points_inbox)):
        ...
        rviz_points.color = ColorRGBA(i+1/(len(points_inbox)+1),1,0,1)
...
# def xyzrgb_array_to_pointcloud2(points_where, points_arr):
    
    
#     msg = PointCloud2()
#     box_3D = []
#     color_list = []
#     for i in range(len(points_where)):
#         box_3D.append(points_arr[points_where[i]])
#         a = np.random.rand(3)
#         color_list.append([a.tolist()]*len(points_where[i]))
...
#     N = len(box_3D)
#     xyzrgb = np.array(np.hstack([box_3D, color_list]), dtype=np.float32)
#     msg.height = 1
#     msg.width = N
...
def listener(camera_info, image_color, velodyne_points,yolo_box,camera_lidar=None):
    ...
    if camera_lidar: 
        ...
        points_pub = rospy.Publisher('before_adaptive',MarkerArray,queue_size = 5)
