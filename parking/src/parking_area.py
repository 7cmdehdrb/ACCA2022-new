...
class ParkingArea(object):
    ...
    def parseArray(self):
        ...
        x1 = m.cos(yaw) * (self.scale.x / 2.0)
        y1 = m.sin(yaw) * (self.scale.y / 2.0)
        x2 = m.sin(yaw) * (self.scale.x / 2.0)
        y2 = m.cos(yaw) * (self.scale.y / 2.0)

        p1 = [self.position.x, self.position.y]
        p2 = [self.position.x + x1 + y1,
              self.position.y + x2 - y2]
        p3 = [self.position.x - x1 - y1,
              self.position.y - x2 + y2]
        p4 = [self.position.x + x1 - y1,
              self.position.y + x2 + y2]
        p5 = [self.position.x - x1 + y1,
              self.position.y - x2 - y2]
